 ...
 * 
 */
public class PersReg2 extends SocialRecommender {
	...
	public PersReg2(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
	...
	protected void buildModel() throws Exception {
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (int u = 0; u < numUsers; u++) {//for all users's friends - mf
				SparseVector uv = socialMatrix.row(u);
				double[] sumNNs = new double[numFactors];
				double[] sumOfSims = new double[numFactors];
				for (int k : uv.getIndex()) {//calculate similarity of all friends with user u - f1
					double suk = 0.0;				
					...
					for (int f = 0; f < numFactors; f++) {
						sumNNs[f] += suk * P.get(k, f);
						sumOfSims[f] += suk;
					}
				
				}//f1 ends
				for (int f = 0; f < numFactors; f++) {
					double diff = P.get(u, f) - sumNNs[f] / sumOfSims[f];
				...
				double[] sumOfsugs = new double[numFactors];
				for (int v : iuv.getIndex()) {
					double sug = 0.0;
					
					if (perssim.equalsIgnoreCase(yes)) {
						sug = perssimilarity(v, u);
					}else {
						sug = similarity(v,u);
					}
					...
					double[] sumOfsufs = new double[numFactors];	
					for (int w : vv.getIndex()) {
						
						double suf = 0.0;
						
						if (perssim.equalsIgnoreCase(yes)) {
							suf = perssimilarity(v, w);							
						}else {
							suf = similarity(v,w);							
						}
						
						for (int f = 0; f < numFactors; f++) {
							sumDiffs[f] += suf * P.get(w, f);
							sumOfsufs[f] += suf;
						}
					}					
						for (int f = 0; f < numFactors; f++)
							PS.add(u, f, -regS * (((sug) * (P.get(v, f) - sumDiffs[f] /sumOfsufs[f]))/sumOfsugs[f]));
